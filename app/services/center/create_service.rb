class Center::CreateService
  def initialize(name:, longitude:, latitude:, website:, logo:, certificate:, registration_number:, tax_id:, email:, phone_number:,
                 current_doctor:, specialty_ids:, social_links:)
    @name = name
    @longitude = longitude
    @latitude = latitude
    @website = website
  ...
  def check_social_links_existed
    ...
    raise error_message if @social_links.nil?

    @parsed_social_links = JSON.parse(@social_links)
    return if @parsed_social_links.length
